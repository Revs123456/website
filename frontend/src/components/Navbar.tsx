...
export default function Navbar() {
  ...
  function parseLinks(json: string | undefined, fallback: { href: string; label: string }[]) {
    if (!json) return fallback;
    try { return JSON.parse(json); } catch { return fallback; }
  }

  const navLinks = parseLinks(s.nav_main_links, NAV_LINKS_DEFAULT);
  const moreLinks = parseLinks(s.nav_more_links, MORE_LINKS_DEFAULT);
  ...
  const moreActive = moreLinks.some((l: { href: string; label: string }) => active(l.href));
            ...
            <div ref={moreRef} style={{ position: 'relative' }}>
              ...
              {moreOpen && (
                <div style={{
                  ...
                  {moreLinks.map(({ href, label }: { href: string; label: string }) => (
                    <Link key={href} href={href}
                      style={{
        ...
        <div style={{ position: 'fixed', inset: 0, zIndex: 40 }}>
          ...
          <div className="mobile-menu-panel" style={{ position: 'absolute', right: 0, top: 0, height: '100%', width: 272, display: 'flex', flexDirection: 'column', background: '#fff', boxShadow: '-4px 0 32px rgba(0,0,0,.12)' }}>
            ...
            <div style={{ flex: 1, padding: 14, display: 'flex', flexDirection: 'column', gap: 2, overflowY: 'auto' }}>
              {[...navLinks, ...moreLinks].map(({ href, label }: { href: string; label: string }) => (
                <Link key={href} href={href}
                  style={{
