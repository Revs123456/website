...
const SOCIALS_DEFAULT = [
  {
    label: 'Instagram',
    href: 'https://www.instagram.com/techchamps_by.rev/',
    color: '#E1306C',
    icon: (
  ...
  {
    label: 'LinkedIn',
    href: 'https://www.linkedin.com/in/revanthkalamshetty/',
    color: '#0A66C2',
    icon: (
  ...
  {
    label: 'Gmail',
    href: 'mailto:connectwithrev@gmail.com',
    color: '#EA4335',
    icon: (
      <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
  ...
  {
    label: 'YouTube',
    href: 'https://www.youtube.com/@RevanthKalamshetty',
    color: '#FF0000',
    icon: (
      <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
...
function buildSocials(s?: PublicSettings) {
  if (!s) return SOCIALS_DEFAULT;
  return SOCIALS_DEFAULT.map(social => ({
    ...social,
    href: (
      social.label === 'Instagram' ? s.social_instagram_url :
      social.label === 'LinkedIn'  ? s.social_linkedin_url :
      social.label === 'Gmail'     ? `mailto:${s.contact_email}` :
      social.label === 'GitHub'    ? s.social_github_url :
      social.label === 'YouTube'   ? s.social_youtube_url :
      social.label === 'WhatsApp'  ? s.social_whatsapp_url :
      social.href
    ) || social.href,
  }));
}
...
export default function Footer({ settings }: { settings?: PublicSettings }) {
  ...
  const siteName = settings?.site_name || 'TechChampsByRev';
  return (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 40, marginBottom: 48 }}>
          ...
          <div style={{ gridColumn: 'span 1' }}>
            <Link href="/" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, marginBottom: 16, textDecoration: 'none' }}>
              <img src="/tc.png" alt={`${siteName} logo`} style={{ width: 28, height: 28, borderRadius: 8, objectFit: 'cover' }} />
              <span style={{ fontWeight: 700, fontSize: 13, color: '#0f172a' }}>{siteName}</span>
            </Link>
            <p style={{ fontSize: 13, color: '#94a3b8', lineHeight: 1.6, marginBottom: 20 }}>{settings?.footer_tagline || 'Helping developers and students launch their tech careers.'}</p>
