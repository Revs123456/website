...
async function getPlans(): Promise<{ plans: any[]; error: boolean }> {
  try {
    const res = await fetch(`${BASE}/services/published`, { cache: 'no-store' });
    if (!res.ok) return { plans: [], error: true };
    const data = await res.json();
    const all: any[] = Array.isArray(data) ? data : (data?.data ?? []);
    // Filter paid plans (price !== 'Free' and price !== 'free')
    const plans = all.filter((p: any) => p.price?.toLowerCase() !== 'free');
    return { plans, error: false };
  } catch { return { plans: [], error: true }; }
}
      ...
      <div style={{ ...wrap, paddingTop: 40, paddingBottom: 80 }}>
        {error ? (
          <div style={{ textAlign: 'center', padding: '80px 0', color: '#dc2626' }}>
            <p style={{ fontSize: 15 }}>Unable to load services. Please try again later.</p>
          </div>
        ) : plans.length === 0 ? (
