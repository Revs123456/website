        ...
        handler: async (response: any) => {
          try {
            ...
            if (err?.message?.toLowerCase().includes('already booked')) {
              api.slots.listAvailable().then(freshSlots => {
                setSlots(freshSlots);
                setSelectedSlot(null);
                setStep('slot'); // go back to time picker with fresh slots
                setError('That slot was just taken. Pick another time below.');
              }).catch(() => {});
            } else {
          ...
          <div className="card" style={{ padding: 28 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: 10 }}>
              {slotsForDate.map(slot => (
                <button
                  key={slot.id}
                  onClick={() => { setSelectedSlot(slot); setError(''); setStep('details'); }}
                  style={{
