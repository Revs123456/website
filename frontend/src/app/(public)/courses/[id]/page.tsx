...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
  ...
  try {
    ...
    return {
      title: `${course.title} — ${course.platform ?? 'Online Course'}`,
      description: course.description?.slice(0, 160) ?? `Learn ${course.title} on ${course.platform}. Level: ${course.level}. Rating: ${course.rating}.`,
      openGraph: {
        title: `${course.title} | TechChampsByRev`,
        description: course.description?.slice(0, 160),
        url: `https://www.techchampsbyrev.in/courses/${id}`,
      },
      alternates: { canonical: `https://www.techchampsbyrev.in/courses/${id}` },
    };
  } catch {
    return { title: 'Course', description: 'View this tech course on TechChampsByRev.' };
  }
}
...
export default async function CourseDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  const courseSchema = {
    ...
    provider: {
      '@type': 'Organization',
      name: course.platform ?? 'TechChampsByRev',
    },
  ...
  return (
    <div style={{ background: '#f8fafc', minHeight: '100vh' }}>
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(courseSchema).replace(/<\/script>/gi, '<\\/script>') }} />
