...
export default function MockInterviewPage() {
  ...
  const expOptions = s.mock_interview_experience_options
    ? s.mock_interview_experience_options.split(',').map(v => v.trim())
    : ['Fresher', '1-3 yrs', '3-5 yrs', '5+ yrs'];
  const roleOptions = s.mock_interview_role_options
    ? s.mock_interview_role_options.split(',').map(v => v.trim())
      ...
      <div style={{ background: '#fff', borderBottom: '1px solid #e2e8f0', paddingTop: 80, paddingBottom: 40 }}>
        ...
        <div style={{ ...wrap, maxWidth: 640 }}>
          <span className="badge badge-blue" style={{ marginBottom: 12, display: 'inline-flex' }}>
            {s.page_mock_interview_badge || 'Free First Session'}
          </span>
          <h1 className="text-display-sm">
            {s.page_mock_interview_title || 'Book a Mock Interview'}
          </h1>
          <p style={{ fontSize: 15, color: '#64748b', marginTop: 8 }}>
            {s.page_mock_interview_subtitle || 'Practice with real engineers. Get hired faster.'}
          </p>
        </div>
      </div>
          ...
          <div>
            ...
            <div style={{
              ...
              <p style={{ fontSize: 13, fontWeight: 700, color: '#1e40af', marginBottom: 4 }}>
                {s.mock_interview_stats_label || '100+ Sessions Conducted'}
              </p>
              <p style={{ fontSize: 12, color: '#3b82f6' }}>
                {s.mock_interview_stats_desc || 'Avg rating 4.8/5 from candidates. 73% received offers within 60 days.'}
              </p>
            </div>
          </div>
