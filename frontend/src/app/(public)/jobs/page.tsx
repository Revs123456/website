...
export default function JobsPage() {
  ...
  const load = () => {
    api.jobs.listPublished().then(d => setJobs(d.filter((j: any) => {
        ...
        <div style={wrap}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'flex-end', justifyContent: 'space-between', gap: 20 }}>
            <div>
              ...
              <h1 className="text-display-sm">{s.page_jobs_title || 'Find your next tech role'}</h1>
              <p style={{ fontSize: 15, color: '#64748b', marginTop: 8 }}>
                {s.page_jobs_subtitle || 'Curated opportunities — remote and onsite.'}
              </p>
            </div>
                  ...
                  >
                    ...
                    <div style={{ flex: 1, minWidth: 200 }}>
                      <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                        ...
                        {/* Phase 4 — AI-free match score (only present for signed-in users with profile data) */}
                        {typeof job.match_score === 'number' && job.match_score >= 40 && (
                          <span
                            title="Match based on your profile — set target role + bio for more accuracy"
                            className="badge"
                            style={{
                              background: job.match_score >= 75 ? '#f0fdf4' : job.match_score >= 60 ? '#eff6ff' : '#fffbeb',
                              color: job.match_score >= 75 ? '#15803d' : job.match_score >= 60 ? '#1d4ed8' : '#b45309',
                              border: `1px solid ${job.match_score >= 75 ? '#bbf7d0' : job.match_score >= 60 ? '#bfdbfe' : '#fde68a'}`,
                            }}
                          >
                    ...
                    <div style={{ display: 'flex', gap: 8, flexShrink: 0, alignItems: 'center' }}>
                      <SaveJobButton jobId={job.id} initiallySaved={job.is_saved} size="md" />
                      <Link
                        href={`/jobs/${job.id}`}
