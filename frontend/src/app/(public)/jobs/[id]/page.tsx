...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
  ...
  try {
    ...
    return {
      ...
      description: `${job.title} role at ${job.company}. Location: ${job.location}. Experience: ${job.experience}. ${job.description?.slice(0, 120) ?? ''}`.trim(),
      openGraph: {
        title: `${job.title} at ${job.company} | TechChampsByRev`,
        description: `Apply for ${job.title} at ${job.company} — ${job.location}`,
        url: `https://www.techchampsbyrev.in/jobs/${id}`,
      },
      alternates: { canonical: `https://www.techchampsbyrev.in/jobs/${id}` },
    };
  } catch {
    return { title: 'Job Listing', description: 'View this tech job on TechChampsByRev.' };
  }
}
...
export default async function JobDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div style={{ background: '#f8fafc', minHeight: '100vh' }}>
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(jobSchema).replace(/<\/script>/gi, '<\\/script>') }} />
