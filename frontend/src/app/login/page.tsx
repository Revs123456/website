...
export default function LoginPage() {
  ...
  useEffect(() => {
    try {
      const raw = localStorage.getItem('tch_auth');
      if (raw) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // Token is in HttpOnly cookie — store only non-sensitive UI info + CSRF token
      localStorage.setItem('tch_auth', JSON.stringify({ role: data.role, email: data.email }));
      // Store CSRF token returned in body (cookie is on API domain, unreadable cross-domain)
      if (data.csrfToken) {
        const { storeCsrfToken } = await import('@/lib/api');
      ...
      <div style={{ width: '100%', maxWidth: 400 }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: 32 }}>
          <Link href="/" style={{ display: 'inline-flex', alignItems: 'center', gap: 10, textDecoration: 'none' }}>
            <div style={{ width: 36, height: 36, borderRadius: 10, background: 'linear-gradient(135deg,#2563eb,var(--brand-violet))', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
