...
export default function AdminDailyTipsPage() {
  ...
  async function load() {
    setLoading(true);
    try {
      const res = await authFetch(`${BASE}/daily-tips`);
  ...
  const save = async (e: React.FormEvent) => {
    ...
    try {
      if (editing) {
        await authFetch(`${BASE}/daily-tips/${editing.id}`, {
          method: 'PATCH',
          body: JSON.stringify(form),
        });
      } else {
        await authFetch(`${BASE}/daily-tips`, {
          method: 'POST',
          body: JSON.stringify(form),
        });
      }
  ...
  const confirmDelete = async () => {
    ...
    setDeleting(true);
    try { await authFetch(`${BASE}/daily-tips/${deleteTarget.id}`, { method: 'DELETE' }); setDeleteTarget(null); load(); }
  ...
  const toggleActive = async (item: any) => {
    try {
      await authFetch(`${BASE}/daily-tips/${item.id}`, {
        method: 'PATCH',
        body: JSON.stringify({ active: !item.active }),
      });
      load();
    } catch {
      setError('Failed to update tip.'); setTimeout(() => setError(''), 5000);
    }
  };
  ...
  return (
    <div>
      ...
      {deleteTarget && (
        <DeleteModal
          title="Delete Tip?"
          name={deleteTarget.tip || deleteTarget.title || ''}
          deleting={deleting}
