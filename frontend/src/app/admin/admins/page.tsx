...
export default function AdminUsersPage() {
  ...
  async function fetchAdmins() {
    try {
      const res = await authFetch(`${BASE}/auth/admins`);
  ...
  async function handleAdd(e: React.FormEvent) {
    ...
    try {
      const res = await authFetch(`${BASE}/auth/create-admin`, {
        method: 'POST',
  ...
  const confirmDelete = async () => {
    ...
    setDeleting(true);
    try { await authFetch(`${BASE}/auth/admins/${deleteTarget.id}`, { method: 'DELETE' }); setDeleteTarget(null); fetchAdmins(); }
    ...
    <>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(340px, 1fr))', gap: 24, alignItems: 'start' }}>
        ...
        <div className="card" style={{ overflow: 'hidden' }}>
          ...
          {loading ? (
            <div style={{ padding: '16px 20px' }}><AdminSkeleton rows={3} /></div>
          ) : admins.length === 0 ? (
