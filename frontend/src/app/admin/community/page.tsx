...
export default function AdminCommunityPage() {
  ...
  async function load() {
    setLoading(true);
    try {
      const res = await authFetch(`${BASE}/community`);
  ...
  const save = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editing) {
        await authFetch(`${BASE}/community/${editing.id}`, {
          method: 'PATCH',
          body: JSON.stringify(payload),
        });
      } else {
        await authFetch(`${BASE}/community`, {
          method: 'POST',
          body: JSON.stringify(payload),
        });
      }
  ...
  const confirmDelete = async () => {
    ...
    setDeleting(true);
    try { await authFetch(`${BASE}/community/${deleteTarget.id}`, { method: 'DELETE' }); setDeleteTarget(null); load(); }
  ...
  const toggleField = async (item: any, field: 'solved' | 'published') => {
    try {
      await authFetch(`${BASE}/community/${item.id}`, {
        method: 'PATCH',
