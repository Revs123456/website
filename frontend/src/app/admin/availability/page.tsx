...
function addThirtyMins(t: string) {
  const [h, m] = t.split(':').map(Number);
...
export default function AdminAvailabilityPage() {
  ...
  // A time T (30-min slot T→T+30) is blocked if it overlaps any existing slot on that date
  ...
  const save = async (e: React.FormEvent) => {
    ...
    try {
      await Promise.all(
        selectedTimes.map(t => api.slots.create({ date: form.date, start_time: t, end_time: addThirtyMins(t) }))
      );
      setShowForm(false);
      setForm({ date: '' });
      setSelectedTimes([]);
  ...
  const saveEdit = async () => {
    ...
    setEditError('');
    try {
      await api.slots.update(editTarget.id, { start_time: editTime, end_time: addThirtyMins(editTime) });
      setEditTarget(null);
      load();
  ...
  const toggle = async (slot: any) => {
    ...
    catch { setSaveError('Failed to update slot.'); setTimeout(() => setSaveError(''), 5000); }
  };
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div style={{ padding: '10px 14px', borderRadius: 8, background: '#eff6ff', border: '1px solid #bfdbfe', display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <span style={{ fontSize: 13, fontWeight: 600, color: '#1e40af' }}>
                  {fmt12(editTime)} – {fmt12(addThirtyMins(editTime))} (30 min)
                </span>
              </div>
