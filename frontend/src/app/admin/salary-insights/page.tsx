...
export default function AdminSalaryInsightsPage() {
  ...
  async function load() {
    setLoading(true);
    try {
      const res = await authFetch(`${BASE}/salary-insights`);
  ...
  const save = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editing) {
        await authFetch(`${BASE}/salary-insights/${editing.id}`, {
          method: 'PATCH',
          body: JSON.stringify(payload),
        });
      } else {
        await authFetch(`${BASE}/salary-insights`, {
          method: 'POST',
          body: JSON.stringify(payload),
        });
      }
  ...
  const confirmDelete = async () => {
    ...
    setDeleting(true);
    try { await authFetch(`${BASE}/salary-insights/${deleteTarget.id}`, { method: 'DELETE' }); setDeleteTarget(null); load(); }
    ...
    <div>
      ...
      {deleteTarget && (
        <DeleteModal
          ...
          name={deleteTarget.role || deleteTarget.title || ''}
          deleting={deleting}
