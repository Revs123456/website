...
export default function AdminTemplatesPage() {
  ...
  async function load() {
    setLoading(true);
    try {
      const res = await authFetch(`${BASE}/resume-templates`);
  ...
  const save = async (e: React.FormEvent) => {
    ...
    try {
      if (editing) {
        await authFetch(`${BASE}/templates/${editing.id}`, {
          method: 'PATCH',
          body: JSON.stringify(form),
        });
      } else {
        await authFetch(`${BASE}/resume-templates`, {
          method: 'POST',
          body: JSON.stringify(form),
        });
      }
  ...
  const confirmDelete = async () => {
    ...
    setDeleting(true);
    try { await authFetch(`${BASE}/templates/${deleteTarget.id}`, { method: 'DELETE' }); setDeleteTarget(null); load(); }
  ...
  const togglePublished = async (item: any) => {
    try {
      await authFetch(`${BASE}/templates/${item.id}`, {
        method: 'PATCH',
        body: JSON.stringify({ published: !item.published }),
      });
      load();
    } catch {
      setError('Failed to update template.'); setTimeout(() => setError(''), 5000);
    }
  };
