...
export default function AdminInterviewQuestionsPage() {
  ...
  async function load() {
    setLoading(true);
    try {
      const res = await authFetch(`${BASE}/interview-questions`);
  ...
  const save = async (e: React.FormEvent) => {
    ...
    try {
      if (editing) {
        await authFetch(`${BASE}/interview-questions/${editing.id}`, {
          method: 'PATCH',
          body: JSON.stringify(form),
        });
      } else {
        await authFetch(`${BASE}/interview-questions`, {
          method: 'POST',
          body: JSON.stringify(form),
        });
      }
  ...
  const confirmDelete = async () => {
    ...
    setDeleting(true);
    try { await authFetch(`${BASE}/interview-questions/${deleteTarget.id}`, { method: 'DELETE' }); setDeleteTarget(null); load(); }
  ...
  const togglePublished = async (item: any) => {
    try {
      await authFetch(`${BASE}/interview-questions/${item.id}`, {
        method: 'PATCH',
        body: JSON.stringify({ published: !item.published }),
      });
      load();
    } catch {
      setError('Failed to update question.'); setTimeout(() => setError(''), 5000);
    }
  };
    ...
    <div>
      ...
      {deleteTarget && (
        <DeleteModal
          ...
          name={deleteTarget.question || deleteTarget.title || ''}
          deleting={deleting}
