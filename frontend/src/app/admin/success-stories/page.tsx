...
export default function AdminSuccessStoriesPage() {
  ...
  async function load() {
    setLoading(true);
    try {
      const res = await authFetch(`${BASE}/success-stories`);
  ...
  const save = async (e: React.FormEvent) => {
    ...
    try {
      if (editing) {
        await authFetch(`${BASE}/success-stories/${editing.id}`, {
          method: 'PATCH',
          body: JSON.stringify(form),
        });
      } else {
        await authFetch(`${BASE}/success-stories`, {
          method: 'POST',
          body: JSON.stringify(form),
        });
      }
  ...
  const confirmDelete = async () => {
    ...
    setDeleting(true);
    try { await authFetch(`${BASE}/success-stories/${deleteTarget.id}`, { method: 'DELETE' }); setDeleteTarget(null); load(); }
  ...
  const togglePublished = async (item: any) => {
    try {
      await authFetch(`${BASE}/success-stories/${item.id}`, {
        method: 'PATCH',
        body: JSON.stringify({ published: !item.published }),
      });
      load();
    } catch {
      setError('Failed to update story.'); setTimeout(() => setError(''), 5000);
    }
  };
